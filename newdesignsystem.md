...
- Panel `.k-panel` (`kiosk/front.ts`): a surface box on 1 px hairline joints, no radius, no shadow; a kicker at the label tier (uppercase, in the domain colour) with a meta line beside it; rows `.k-fr` on hairlines, each a lead cell (a clock, a distance, a day word or a line badge) at the supporting tier, a title at the supporting tier clamped to two lines, one line of context at the hint tier (may ellipsise); a figure at the main tier where a number is the point (tomorrow's range); the credit at the credit tier pushed to the foot. Rows a panel does not hold whole are hidden from the foot up by measurement, never clipped. The five panels: tonight (two columns of events on a wall), weather, city, promet (the stop's lines, the last departures from 20:00, ZET's notice), around (closures by distance, works). The map is one panel among them, 1500 m across.
...
| Kiosk | `scenes.ts` chapter rotation + two value tiles | `front.ts` (five panels read from the teaser's nine modules) and `field.ts` (the map panel, mounted once) replace `scenes.ts`; `invitation.ts` lays the panels out as the city's front page. |
