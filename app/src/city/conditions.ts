...
export function conditionsMarkup(ctx:LayerContext):string {
  ...
  return `<section class="city-condition wx-sec"><h3>${ct(ctx.i18n,'air')}</h3>${rows||`<p>${ct(ctx.i18n,'unavailable')}</p>`}<p class="city-meta">${e(ctx.i18n.t('city.airMethod'))}</p></section>
...
export function consultationsMarkup(ctx:LayerContext):string {
  ...
  return `<section class="city-condition cv-sec" id="cv-consultations"><h3>${e(ctx.i18n.t('civic.consultations'))}</h3><p class="city-meta">${e(ctx.i18n.t('city.consultationsNote'))}</p>
