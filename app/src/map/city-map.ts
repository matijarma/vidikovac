...
export function createCityMap(options: CityMapOptions, deps: CityMapDeps = {}): CityMapHandle {
  ...
  function installNetwork(network: Network | null): void {
    ...
    nextPushAt = -Infinity;
    if (styled && lib) {
      setData(lib.SOURCES.network, net ? networkToGeoJson(net) : { type: 'FeatureCollection', features: [] });
      setData(lib.SOURCES.stops, net ? stopsToGeoJson(net) : { type: 'FeatureCollection', features: [] });
      applyOverlays();
    }
