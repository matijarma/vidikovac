...
// styled before this module has even been fetched. The entry adds only what a
// link cannot: the toast sheet for bootPage's queue, and the one Manrope
// family as a dynamic import, so the font sheet arrives after the first paint
// and never in front of it. With the theme module in <head> these are the
// page's only two scripts, both external (R-16).
