// Inline SVG sprite, Lucide icons (ISC, https://lucide.dev). mountIconSprite()
// once per document, then iconMarkup(name) anywhere. Stroke styling lives in
// base.css `.icon`. Icons are always beside text or given a label; none carries
// meaning alone.
...
export const ICON_NAMES = [
  ...
  'printer', 'qr-code', 'radio', 'refresh-cw', 'search', 'share-2', 'shield', 'siren', 'sun', 'sunrise',
  'sunset', 'thermometer', 'ticket', 'sliders-horizontal', 'tram-front', 'triangle-alert', 'users', 'wind', 'x', 'zap',
] as const;
...
export function iconMarkup(name: IconName, label?: string, className = 'icon'): string {
  if (label) {
    return `<svg class="${escapeAttribute(className)}" role="img" aria-label="${escapeAttribute(label)}"><use href="#icon-${name}"></use></svg>`;
  }
  return `<svg class="${escapeAttribute(className)}" aria-hidden="true"><use href="#icon-${name}"></use></svg>`;
}
