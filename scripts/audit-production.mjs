...
const METRICS_FN = ({ shellRoot }) => {
  ...
  return {
    ...
    // Two elements carry kiosk-lines (R-KP8, contract 8): the lagano board inside the field and the lines panel's
    // list; each is read under its own parent, never bare, so a lagano capture with both never counts one for the other.
    kiosk: kioskField ? {
      panels: Object.fromEntries([...document.querySelectorAll('[data-testid^=kiosk-panel-]')].map((el) => [el.getAttribute('data-panel'), [...el.querySelectorAll('li.k-fr')].filter((li) => !li.hidden).length])),
      boardPresent: Boolean(document.querySelector('[data-testid=kiosk-live] [data-testid=kiosk-lines]')),
      linesPanelPresent: Boolean(document.querySelector('[data-say=transit] [data-testid=kiosk-lines]')),
    } : null,
