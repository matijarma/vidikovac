#!/usr/bin/env node
// The inline stylesheets of /hitno and /open/, generated from tokens.css.
//
// Both pages answer under `default-src 'none'` with one inline <style> and no
// other request (worker/security-headers.ts), so they cannot load
// app/src/ui/tokens.css. Until wave 6 they carried a hand-copied palette that
// drifted from the app's. This script copies the sRGB literals of the thirteen
// colours the pages paint into worker/hitno/style.generated.ts, so the palette
// has one source; test/open/hitno-style.test.ts fails when the committed copy
// is behind, and test/app/contrast.test.ts measures the copy, not the source.
//
//   node scripts/build-hitno-style.mjs      (npm run build runs it first)
//
// The layout rules live here as well, next to the colours they use, because
// the generated file is not edited by hand. The pages keep the system font
// stack (R-K2): identity comes from the palette, the spacing and the badge
// grammar, never from a font request.
import { readFileSync, writeFileSync } from 'node:fs';
import { dirname, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '..');
export const TOKENS_PATH = resolve(ROOT, 'app/src/ui/tokens.css');
export const OUTPUT_PATH = resolve(ROOT, 'worker/hitno/style.generated.ts');

/** `[token name in tokens.css, custom property on the page]`, the thirteen colours the pages need. */
...
export function readPalette(tokensCss, scheme) {
  const out = {};
  for (const [token, property] of PALETTE) {
    const m = new RegExp(`--palette-${scheme}-${token}:\\s*(#[0-9a-fA-F]{6}|rgba?\\([^)]*\\))\\s*;`).exec(tokensCss);
    if (!m) throw new Error(`tokens.css has no sRGB literal for --palette-${scheme}-${token}`);
    out[property] = m[1].toLowerCase();
  }
  return out;
}

const declarations = (palette) => Object.entries(palette).map(([property, value]) => `--${property}:${value}`).join(';');

/** Custom properties only: the palette, radii, targets and the system stack. */
function paletteBlock(light, dark) {
  return [
    `:root{color-scheme:light dark;${declarations(light)};--r-lg:1rem;--r-md:.75rem;--target:2.75rem;--target-primary:3rem;` +
      `--font:system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif}`,
    `@media (prefers-color-scheme:dark){:root{${declarations(dark)}}}`,
  ].join('\n');
}

// Shared by both pages: the canvas, links, the skip link, the wordmark with
// its accent question mark, the h1 at display (1.75rem), the footer set.
// Row titles are 600 because the system stack has no 500 face on Windows
// (Segoe UI: 400, 600, 700); heads 700; prose 400.
const BASE = `
...
// sections as content separated by hairlines, the emergency numbers as the
// one set of tiles (the objects), 44 px targets on every link and summary.
const HITNO = `
...
// /open/: a 46rem reading measure, one article per dataset separated by
// hairlines, the downloads as 44 px buttons, the offer as a section.
const OPEN = `
...
/** Backtick, `${` and backslash escaped so the CSS can sit inside a TypeScript template literal. */
const literal = (css) => css.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${');

/** The generated module's source and the palettes it embeds. */
export function buildStyles(tokensCss) {
  const light = readPalette(tokensCss, 'light');
  const dark = readPalette(tokensCss, 'dark');
  const palette = paletteBlock(light, dark);
  const source = [
    ...
    `export const PAGE_PALETTE = \`${literal(palette)}\n\`;`,
    '',
    '/** The one inline stylesheet of /hitno. */',
    `export const HITNO_STYLE = \`${literal(palette)}${literal(BASE)}${literal(HITNO)}\`;`,
    '',
    '/** The one inline stylesheet of /open/. */',
    `export const OPEN_STYLE = \`${literal(palette)}${literal(BASE)}${literal(OPEN)}\`;`,
    '',
  ].join('\n');
...
function main() {
  const { source } = buildStyles(readFileSync(TOKENS_PATH, 'utf8'));
  let current = null;
  try {
  ...
  if (current === source) {
    console.log('build-hitno-style: worker/hitno/style.generated.ts is current');
    return;
  }
  writeFileSync(OUTPUT_PATH, source);
  console.log('build-hitno-style: wrote worker/hitno/style.generated.ts');
}
