...
/** The fixture README, deterministic for a given input so a re-run leaves git clean. */
export function renderReadme(s) {
  ...
  const lines = [
    ...
    '',
    'Generated by `scripts/frames-sample.mjs`; a re-run rewrites this file and the frames, so edit the script, not this file.',
    '',
    '## Source and licence',
    '',
    ...
    '',
    `The frames name trips of ZET's static GTFS feed **${s.feedVersion ?? 'unknown'}**: ${join} (feed ${s.feedVersion ?? 'unknown'}). Service prefixes of the tram trip ids: ${s.services.join(', ') || 'none'}. The network artefact of the same feed is \`${NETWORK_PATH}\`, graphHash \`${s.graphHash ?? 'unknown'}\`.`,
    '',
...
export async function main({
  ...
  const refusal = refusalFor(out, input);
  ...
  for (const name of names) {
    ...
    const frame = filterFrame(source, keep);
    const data = args.allModes ? source : frame.bytes;
