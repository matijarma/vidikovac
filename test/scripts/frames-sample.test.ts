...
describe('frames-sample README regeneration', () => {
  ...
  it('the committed README cites the committed artefact: --readme-only ran after the last network regeneration', async () => {
    // The two are regenerated by different scripts (gtfs-shapes.mjs writes the artefact, frames-sample.mjs
    // the README), so a rail-graph change merged without `--readme-only` leaves the README naming a
    // graph that no longer exists. Read both from the tree, not from a render.
    const committed = await readFile(join(FIXTURE, 'README.md'), 'utf8');
    const network = JSON.parse(await readFile(join(REPO, 'app/public/data/zet-network.json'), 'utf8'));
    expect(network.graphHash, 'the artefact carries a graphHash').toMatch(/^[0-9a-f]{16}$/);
    expect(committed).toContain(`graphHash \`${network.graphHash}\``);
  });
