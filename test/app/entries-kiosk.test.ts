...
describe('the kiosk map renderer at boot', () => {
  it('lets ?prikaz= override the preference, preserves it through URL cleanup, drops the retired chapter pin, and otherwise reads the store', async () => {
    ...
    expect(vi.mocked(mountKiosk).mock.lastCall?.[1]).toMatchObject({ mapMode: 'schema', lightweight: false });
    expect(location.hash).toBe('');
    expect(location.search).toBe('?prikaz=shema');
