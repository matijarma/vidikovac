...
import {
  ...
  SENTENCE_COPY_HR, type RotatingSentence, type SentenceFactsInput, type SentenceNearbyRow,
...
describe('sentence sequence: one fact, once in ten minutes', () => {
  ...
  it('restates the fact on screen in fresh words instead of blanking, and never returns to a shown fact', () => {
    ...
    const y = sentence('Muzej prikazuje izložbu broj 1.', { refs: ['event:y'], kicker: 'kultura' });
    const z = sentence('Muzej prikazuje izložbu broj 2.', { refs: ['event:z'], kicker: 'vrijeme' });
    const x1 = at(sentence('Muzej prikazuje izložbu broj 3.', { refs: ['dep:a'], kicker: 'promet', validUntil: NOW + 50_000 }), 'departure:6');
    const x2 = at(sentence('Muzej prikazuje izložbu broj 4.', { refs: ['dep:b'], kicker: 'promet' }), 'departure:6');
    // A model sentence citing the shown fact y is that fact in other words.
    const y2 = sentence('Muzej prikazuje izložbu broj 5.', { refs: ['event:y'], kicker: 'kultura', origin: 'model' });
    const seq = createSentenceSequence({ rhythmMs: 20_000 });
