...
describe('i18n orphans: every catalogue leaf is read (WP5 A1)', () => {
  ...
  it('the catalogue holds 1044 Croatian leaves and 1016 English ones', () => {
    ...
    // kvart.walkMinutes): 1,044 hr, 1,016 en. A new leaf changes this number on purpose, with its reader.
    expect(leaves.length).toBe(1044);
    expect(leafKeys(en).length).toBe(1016);
