...
describe('decodeNetwork', () => {
  it('decodes version 3 into the superset the client reads: edges, tram shapes rebuilt from them, bus polylines, paths with offsets and their served stops, terminal stops, stops at exact arcs on every shape through their edges, nextStop; rejects a version 2 artefact; loads only outside lightweight mode', async () => {
    ...
    // F8: the served list -- the platforms the path's own trips call at, in
    // arc order, metres from the decimetres on the wire. C lies on E1 but S1
    // never calls there, so the shape path's list stops at B.
    expect(shapePath.served).toEqual([{ stop: 0, s: 4.7 }, { stop: 1, s: 33.3 }]);
    expect(synthetic.served).toEqual([{ stop: 1, s: 10 }, { stop: 2, s: 15 }]);
    ...
    const fetchSpy = vi.fn(async (url: string) => {
      ...
      return { ok: true, json: async () => rawArtefactV3() } as Response;
    });
