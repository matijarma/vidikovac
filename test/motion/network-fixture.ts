...
// produces at version 3, shared by network.test.ts and graph.test.ts.
...
// arithmetic the decoder uses, so comparisons are exact. Stop C is a phantom
// on the shape path: it lies on E1 but S1's trips never call there, so S1's
// `served` list names A and B alone (F8).
...
export function rawArtefactV3() {
  return {
    ...
    shapes: {
      ...
      e: [[0, 1], []],
      len: [999.9, 888.8],
      // [stopIdx, decimetres along the path]: A at 4.7 m, B 10 m into E1
      // (E0 is 23.3 m long). A bus shape runs no path and serves nothing.
      served: [[[0, 47], [1, 333]], []],
    },
    paths: { id: ['path:R1:1:deadbeef'], route: ['R1'], dir: [1], e: [[1]], stops: [['B', 'C']], served: [[[1, 100], [2, 150]]] },
    stops: {
      id: ['A', 'B', 'C', 'D'],
      ...
      terminal: [1, 0, 1, 0], // A and C end a trip somewhere in the feed
    },
