...
*Posluženi popis stajališta.* Uz geometrijske veze stajališta na bridove (40 m, koje čitaju matcher i krugovi stajališta na gradskoj karti) svaka staza sada nosi `served` -- perone na kojima **njezini vlastiti uzorci stvarno staju**, po luku -- a svako stajalište nosi `terminal`. Motor (planer, registar, učenje, procjena brzine, objava) čita isključivo `served` kroz `stopsOnPath` iz `shared/motion/graph.ts`, pa više ne knjiži zadržavanje na peronu suprotnog smjera tri metra dalje ni na peronu druge linije na istim tračnicama. Na snimljenom danu 17. rujna takvih je fantomskih zapisa bilo **6.410 od 10.599**; sada ih je **0 od 11.004** (3.302 poslužena).
...
*Imenovane iznimke gradnje* stoje u `scripts/gtfs-shapes-overrides.json`, svaka s razlogom i mjerom: `unreachableStops` (na feedu 000395 prazan) i `longLegs` -- dva hopa Botanički vrt → Zrinjevac linija 6 i 9, gdje istočni kolosijek Mihanovićeve završava, a sjeverni Trga kralja Tomislava počinje 6,79 m dalje, pa se dvije polilinije nikad ne sijeku i graf skretanje ne može izraziti. Bez unosa gradnja **pada**: predugi obilazak je u pravilu nedostajući čvor, ne ruta.
...
- **Odnos je ljepljiv.** Ne kida ga ni zbijanje (tada je red najvažniji, a dokazi najslabiji) ni metarski mikro-brid koji ostaje iza čvora na križanju: odnos je kartoteciran po **tračnicama koje dvoje dijele**, ne po vođinu trenutnom bridu.
...
- **Ponovno kartoteciranje**: sljedbenik kojemu se u međuvrijeme umetnuo bliži tramvaj ne preokreće odnos nego se prijavljuje iza onoga neposredno ispred sebe, čiji razmak doista veže.
...
tipa „6·11·12·14”, najviše `CLUSTER_MAX_NUMBERS` = 4 broja pa „+n”, i s popisom članova. Skupina se
...
Shema crta iste oznake u istoj tinti (`schema-paint.ts` čita `PILL_INKS` iz istog modula), s istim
