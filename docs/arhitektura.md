...
ZET-ov feed (GTFS-Realtime, otkucaj 10 s) šalje samo `trip{tripId, routeId, startDate}`, `position{lat, lon}`, `vehicle{id}` i vlastiti `timestamp` -- bez smjera i bez brzine (R-P3, `worker/feed/modules/zet-rt.ts`). Klijent (`app/src/motion/model.ts`) svaki takav zapis presavija u vlastitu povijest vozila (`update`) i tek na svakoj slici (`step`, u ritmu zaslona) izračunava gdje ono zapravo jest. Statičnu geometriju linija spaja klijent, ne Worker: skup kandidata za `tripId` je malen (medijan 3, najviše 17 oblika po liniji), a raspoređeni oblik nije dokaz gdje je vozilo zapravo skrenulo -- samo ostatak (`residual`) klijentskog spajanja odlučuje o načinu "slobodne ravnine". Mrežni artefakt (`app/public/data/zet-network.json`, opisan u `docs/izvori.md`) dohvaća se odvojeno od ulaznog paketa (nakon prvog iscrtavanja, nikad u lagano načinu, R-L4) i dekodira ga `app/src/motion/network.ts` iz stupčastog (struct-of-arrays) zapisa s lančano-delta kodiranim točkama -- ne iz skice kakvu je prvi nacrt zadatka T4 pretpostavljao (R-T3).

**Blizanac (od 16. rujna 2026, faza A; plan `twin-engine`).** ZET-ov feed više ne čita Worker na svaki zahtjev, nego jedan Durable Object, `TwinDO` (`worker/do/twin-do.ts`), koji se budi vlastitim alarmom u ritmu feeda (svakih 10 s, uvjetnim zahtjevom s `If-None-Match`, `worker/twin/clock.ts`), svako očitanje vozila slaže u kratku povijest tog vozila (12 očitanja, `worker/twin/history.ts`), spaja `tripId` sa statičnim GTFS-om u smjer, odredište, oblik i blok (indeks `app/public/data/zet-trips.json` gradi `scripts/gtfs-trips.mjs`; blizanac ga jednom po verziji feeda kopira u svoj SQLite), pamti stanje u jednom SQLite retku po otkucaju (pa izbacivanje iz memorije između dvaju alarma ništa ne gubi), sprema sirove okvire u R2 (`vidikovac-feed`, sedam dana) za ponavljanje i ocjenu modela, i objavljuje modul `zet-rt` s poviješću (`motion.history`) i spojem (`direction`, `headsign`, `shapeId`, `nextStopId`, `delaySeconds`). Predmemorija modula vrijedi do `validUntil` (idući otkucaj), klijent anketira 1,5 s poslije njega, a ostali moduli zadržavaju svoj ritam od 30 s. Model na klijentu prima povijest kao niz očitanja, pa se vozilo od prve slike giba i gleda u smjeru pruge, oblik linije mu je onaj s vožnje, a ne najbliži (čime prestaje mijenjanje kolosijeka koje se čitalo kao vožnja unatrag), a kartica piše odredište s vožnje umjesto "smjer nepoznat". Faza B premješta i sam proračun gibanja u blizanca (graf pruga, planovi, zakoni bez preticanja i bez vožnje unatrag, ocjena unatrag); do tada vrijedi tablica konstanti dolje. Zdravlje blizanca čita se na `/stats` (`twin_tick` po ishodu i startu, `static_watch` za noviji statični GTFS).
