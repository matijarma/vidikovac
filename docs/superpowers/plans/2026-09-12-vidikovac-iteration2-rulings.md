...
R-F10a (ratifies F7's ruling 1) A fix taken within the dead zone of a stop charges half a dwell to each adjacent interval. F7 measured the three candidate rules against the brief's own tram: strict-interior reads 8.33 m/s, whole-dwell 15, half-and-half exactly 10.00 and 7.00; only the half rule is unbiased. Its cost is that a tram that never stops is charged dwells it never made, and one such run regressed from 190 to 292 m. ZET trams stop at every platform, so the dwelling truth model is the one the screen serves.

P-4 (deferred beyond iteration 2) With F7, dwelling-tram p95 lag is 50 to 158 m against the 120 m target; held share rose to 28 to 47 percent because a correct cruise speed now reaches every gate on time. The remaining lag is the gate hold itself. The next lever is the gate, not the estimate: release when fix latency alone says the tram must have left, or shorten the dwell. Not pulled in iteration 2; recorded for the next one with all three measured columns in test/motion/model.test.ts.
